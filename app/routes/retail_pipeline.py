...
router = APIRouter(prefix="/admin/retail-pipeline", tags=["admin-retail-pipeline"])
...
def derive_week_label(filename: str) -> str:
    ...
    stem = re.sub(r"(?i)^retail\s+", "", stem).strip()
    return stem
...
def replace_training_data_from_long_df(long_df: pd.DataFrame, db: Session):
    db.query(FishTrainingPrice).delete(synchronize_session=False)
    db.commit()
...
def finalize_upload(
    ...
    status = read_status()
    status["retail_lastUploadFilename"] = None
    status["retail_lastUploadDate"] = datetime.now().strftime("%Y-%m-%d")
    write_status(status)
    ...
    return {
        "message": "Retail file accepted successfully",
...
def preprocess_merge(
    ...
    status = read_status()
    status["retail_lastMergedFilename"] = None
    status["retail_lastWeekLabel"] = week_label
    write_status(status)
    ...
    return {
        "message": "Retail weekly data saved to DB and merged in memory",
...
def preprocess_filter(
    ...
    wide_df = build_wide_from_actual_db(db)
    if wide_df.empty:
        raise HTTPException(status_code=404, detail="No retail actual data found in database")

    filtered_df, summary_df = filter_fish_with_50pct_rule(wide_df)

    status = read_status()
    status["retail_lastFilteredFilename"] = None
    status["retail_lastFilteredSummaryFilename"] = None
    write_status(status)
    ...
    log_upload_action(
        db=db,
        filename=None,
        week_label=status.get("retail_lastWeekLabel"),
    ...
    return {
        "message": "Retail 50% filter applied",
        "stats": {
...
def preprocess_interpolate(
    ...
    status = read_status()
    status["retail_lastInterpolatedFilename"] = None
    write_status(status)

    week_cols = [c for c in interpolated_df.columns if c not in ["Sinhala Name", "Common Name"]]

    return {
        "message": "Retail interpolation completed",
        "stats": {
            "fishCount": fish_count,
...
def preprocess_long_format(
    ...
    if long_df.empty:
        raise HTTPException(status_code=400, detail="Failed to generate retail long format dataset")

    status = read_status()
    status["retail_lastLongFilename"] = None
    status["retail_lastPreprocessDate"] = datetime.now().strftime("%Y-%m-%d")
    write_status(status)

    return {
        "message": "Retail long format dataset generated in memory",
        "stats": {
...
def sync_long_to_db(
    ...
    add_pipeline_log(
        ...
        status="Completed",
        notes=f"{len(long_df)} rows synced from in-memory retail dataset",
    )

    return {
        "message": "Retail long format training dataset stored in DB successfully",
...
def train_hybrid_model_route(
    ...
    try:
        result = get_deployed_model_info(price_type="retail")

        status = read_status()
        status["retail_lastModelStatus"] = "Ready"
        status["retail_lastModelName"] = result["modelName"]
        status["retail_lastModelTrainedAt"] = result["trainedAt"]
        status["retail_lastModelSource"] = "Colab"
        status["retail_lastModelWeights"] = result["bestWeights"]
        status["retail_lastModelAnnAlpha"] = result["annAlpha"]
        status["retail_lastModelMetrics"] = result.get("metrics", {})
        write_status(status)
        ...
        return {
            "message": "Retail deployed hybrid ANN + XGBoost model is ready",
...
def predict_next_week(
    ...
    try:
        training_rows = db.query(FishTrainingPrice).all()
        if not training_rows:
            raise HTTPException(
                status_code=404,
                detail="Retail training data not found in DB. Generate and sync long format first."
            )
        ...
        if long_data.empty:
            raise HTTPException(
                status_code=404,
                detail="Retail training data is empty."
            )
        ...
        final_model = result.get("modelName", "Retail_Hybrid_ANN_XGBoost")
        ...
        add_pipeline_log(
            ...
            status="Completed",
            notes=f"{row_count} retail predictions saved to DB. Batch: {batch_id}",
        )

        status = read_status()
        status["retail_lastPredictionFilename"] = None
        status["retail_lastPredictionBatchId"] = batch_id
        status["retail_lastPredictionDate"] = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
        status["retail_lastModelName"] = final_model
        status["retail_latestPredictionWeek"] = output_week
        write_status(status)

        return {
            "message": "Retail next week predictions generated and saved to DB successfully",
            "date": output_week,
            "rowCount": row_count,
            "batchId": batch_id,
            "savedToDb": True,
        ...
        raise HTTPException(status_code=500, detail=f"Retail prediction failed: {str(e)}")
...
def publish_predictions(
    ...
    status = read_status()
    batch_id = status.get("retail_lastPredictionBatchId")

    if not batch_id:
        raise HTTPException(status_code=404, detail="No retail prediction batch found to publish.")

    rows = db.query(PredictionResult).filter(PredictionResult.batch_id == batch_id).all()

    if not rows:
        raise HTTPException(status_code=404, detail="Retail prediction rows not found for latest batch.")
    ...
    add_pipeline_log(
        ...
        status="Completed",
        notes=f"Published retail batch: {batch_id} ({len(rows)} predictions)",
    )

    status["retail_lastPublishedBatchId"] = batch_id
    status["retail_lastPublishedAt"] = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
    write_status(status)

    return {
        "message": "Retail predictions published successfully",
        "batchId": batch_id,
...
def get_activity_logs(
    ...
    for row in logs:
        ...
        if not action_text.lower().startswith("retail"):
            continue
